...
const CDX = {
  ...
  components: [
    { "bom-ref": "ref-react", type: "library", name: "react", version: "18.3.9", purl: "pkg:npm/react@18.3.9" },
    {
      ...
      version: "1.23.3",
      purl: "pkg:npm/%40remix-run/router@1.23.3",
    },
...
const SPDX = {
  ...
  relationships: [
    { spdxElementId: "SPDXRef-root", relatedSpdxElement: "SPDXRef-react", relationshipType: "DEPENDS_ON" },
  ],
};
...
describe("parsePurl", () => {
  ...
  it("drops qualifiers and subpaths, which are not part of identity", () => {
    expect(parsePurl("pkg:npm/react@18.3.9?arch=x86#src/lib")).toEqual({
      type: "npm",
      name: "react",
      version: "18.3.9",
    });
...
describe("CycloneDX", () => {
  ...
  it("falls back to everything when the graph is absent or matches nothing", () => {
    ...
    const brokenGraph = parseSbom(
      JSON.stringify({ ...CDX, dependencies: [{ ref: "root-ref", dependsOn: ["nope"] }] }),
    )!;
