...
export function quadrantSVG(deps: DepReport[], opts: ChartOptions = {}): string {
  ...
  parts.push(text(PAD.l, 22, opts.title ?? "dependency drift × viability", { size: 13, fill: TEXT, anchor: "start" }));
  ...
  for (const d of plot) {
    ...
    parts.push(
      `<circle cx="${round2(cx)}" cy="${round2(cy)}" r="4.5" fill="${c}" fill-opacity="0.45" stroke="${c}" stroke-width="1"/>`,
    );
  ...
  if (skipped > 0) {
    parts.push(
      text(W - PAD.r, 22, `${skipped} not plotted (no registry data)`, { size: 10, fill: MUTED, anchor: "end" }),
    );
  }
