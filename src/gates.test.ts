...
describe("gateFailures", () => {
  ...
  describe("the ratchet", () => {
    ...
    it("compares at the two decimals every surface reports", () => {
      expect(
        gateFailures(report([dep({ libyearsBehind: 3.52 })]), { maxLibyearsIncrease: 0.42, baselineLibyears: 3.1 }),
      ).toEqual([]);
      expect(
        gateFailures(report([dep({ libyearsBehind: 0.10000000000000002 })]), {
          maxLibyearsIncrease: 0,
          baselineLibyears: 0.1,
        }),
      ).toEqual([]);
    });
  ...
  it("reports every breached gate, not just the first", () => {
    ...
    expect(
      gateFailures(r, { maxLibyears: 1, maxReplace: 0, maxLibyearsIncrease: 0, baselineLibyears: 1 }).map(
        (f) => f.gate,
      ),
