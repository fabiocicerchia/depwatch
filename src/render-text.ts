...
export function table(r: Report, t: Thresholds): string {
  ...
  out.push(
    `quadrants: replace ${counts.replace}  upgrade ${counts.upgrade}  watch ${counts.watch}  healthy ${counts.healthy}`,
  );
  ...
  if (pulseOnly.length > 0)
    out.push(
      `${pulseOnly.length} dep(s) scored on pulse and viability only — no comparable version series (drift shown as —)`,
    );
